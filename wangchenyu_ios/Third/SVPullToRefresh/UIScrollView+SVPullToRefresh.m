...
static CGFloat const HeyHaRefreshViewHeight = 120;
...
-(void)addHeyHaImageViewToRefreshWithActionHandler:(void (^)(void))actionHandler position:(SVPullToRefreshPosition)position imageUrl:(NSString *)url andSign:(NSString *)sign
{
    if(!self.pullToRefreshView) {
        //
        [[NSNotificationCenter defaultCenter] addObserverForName:@"REQUEST_FINISHED" object:nil queue:[NSOperationQueue mainQueue] usingBlock:^(NSNotification *note) {
            if(self.pullToRefreshView.showHeyHaImageView && !self.pullToRefreshView.showHeyHaImageFirst){
                 [self.pullToRefreshView stopAnimating];
            }else if(!self.pullToRefreshView.showHeyHaImageView){
                [self.pullToRefreshView stopAnimating];
            }
        ...
        switch (position) {
            case SVPullToRefreshPositionTop:
                if ([@"" isEqualToString:[NSString CheckWithString:url]]){
                     yOrigin = -SVPullToRefreshViewHeight;
                }else
                {
                    yOrigin = -HeyHaRefreshViewHeight;
                }
        ...
        if ([@"" isEqualToString:[NSString CheckWithString:url]]) {
            ...
            view.showHeyHaImageView = NO;
        }else{
            view.frame = CGRectMake(0, yOrigin, self.bounds.size.width, HeyHaRefreshViewHeight);
            [view.heyhaImageView sd_setImageWithURL:[NSURL URLWithString:url] completed:^(UIImage *image, NSError *error, SDImageCacheType cacheType, NSURL *imageURL) {
            }];
            if([@"teacher" isEqualToString:[NSString CheckWithString:sign]] || [@"parent" isEqualToString:[NSString CheckWithString:sign]]){
                view.showHeyHaTeacher = YES;
                
            }else{
                view.showHeyHaTeacher = NO;
            }
            view.showHeyHaImageView = YES;
            view.showHeyHaImageFirst = YES;
...
- (void)addPullToRefreshWithActionHandler:(void (^)(void))actionHandler position:(SVPullToRefreshPosition)position {
    [self addHeyHaImageViewToRefreshWithActionHandler:actionHandler position:position imageUrl:nil andSign:nil];
}
    ...
    if(!showsPullToRefresh) {
        ...
        if (!self.pullToRefreshView.isObserving) {
            ...
            switch (self.pullToRefreshView.position) {
                case SVPullToRefreshPositionTop:
                    if (self.pullToRefreshView.showHeyHaImageView) {
                        yOrigin = -HeyHaRefreshViewHeight;
                    }
                    else{
                        yOrigin = -SVPullToRefreshViewHeight;
            ...
            if (self.pullToRefreshView.showHeyHaImageView) {
                self.pullToRefreshView.frame = CGRectMake(0, yOrigin, self.bounds.size.width, HeyHaRefreshViewHeight);
                self.pullToRefreshView.heyhaImageView.frame = CGRectMake(0, 0, self.bounds.size.width, self.bounds.size.height);
            }else{
                self.pullToRefreshView.frame = CGRectMake(0, yOrigin, self.bounds.size.width, SVPullToRefreshViewHeight);
            }
...
- (void)layoutSubviews {
    ...
    if(hasCustomView) {
        ...
        if(subtitleSize.height > 0){
            ...
            CGFloat titleY = minY;
            CGFloat subTitleY = titleY + titleSize.height + marginY;
            if (self.showHeyHaImageView) {
                titleY = titleY/2*3;
                subTitleY = subTitleY/2*3;
            }
            self.titleLabel.frame = CGRectIntegral(CGRectMake(labelX, titleY, titleSize.width, titleSize.height));
            self.subtitleLabel.frame = CGRectIntegral(CGRectMake(labelX, subTitleY, subtitleSize.width, subtitleSize.height));
        }else{
        ...
        if (self.showHeyHaImageView) {
            self.arrow.frame = CGRectMake(arrowX,
                                          (self.bounds.size.height / 2)/2*3 - (self.arrow.bounds.size.height / 2) -3,
...
- (void)setScrollViewContentInsetForLoading {
    ...
    switch (self.position) {
        case SVPullToRefreshPositionTop:
            //备注MIN
            if (self.showHeyHaImageView && self.showHeyHaImageFirst && self.showHeyHaTeacher) {
                currentInsets.top = MIN(HeyHaRefreshViewHeight, self.originalTopInset + self.bounds.size.height);
            }else if (self.showHeyHaImageView && self.showHeyHaImageFirst && !self.showHeyHaTeacher){
...
- (void)observeValueForKeyPath:(NSString *)keyPath ofObject:(id)object change:(NSDictionary *)change context:(void *)context {
    if([keyPath isEqualToString:@"contentOffset"])
        ...
        if (self.showHeyHaImageView){
            self.frame = CGRectMake(0, yOrigin, self.bounds.size.width, HeyHaRefreshViewHeight);
            self.heyhaImageView.frame = CGRectMake(0, 0, self.bounds.size.width, self.bounds.size.height);
        }else{
            self.frame = CGRectMake(0, yOrigin, self.bounds.size.width, SVPullToRefreshViewHeight);
        }
...
- (void)scrollViewDidScroll:(CGPoint)contentOffset {
    if(self.state != SVPullToRefreshStateLoading) {
        ...
        switch (self.position) {
            case SVPullToRefreshPositionTop:
                if (self.showHeyHaImageView) {
                    scrollOffsetThreshold = self.frame.origin.y/4 - self.originalTopInset;
                }else{
                    scrollOffsetThreshold = self.frame.origin.y/2 - self.originalTopInset;
                }
...
-(UIImageView *)heyhaImageView
{
    if (_heyhaImageView == nil) {
        _heyhaImageView = [[UIImageView alloc]init];
        _heyhaImageView.contentMode = UIViewContentModeScaleToFill;
        [self addSubview:_heyhaImageView];
    }
    return _heyhaImageView;
}
