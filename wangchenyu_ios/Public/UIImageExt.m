...
+(NSData *)makeImageData:(UIImage *)image withByteNum:(NSInteger)byteNum withQuality:(double)quality{
    if(image ==nil || ![image isKindOfClass:[UIImage class]])
        return nil;
    image               = [UIImage fixOrientation:image];//翻转方向
    // 原始数据
    NSData *imgData = UIImageJPEGRepresentation(image, 1.0);
    // 原始图片
    UIImage *result = [UIImage imageWithData:imgData];
    imgData = UIImageJPEGRepresentation(result, quality);
    /*
    while (imgData.length > byteNum*1024) {
        if (imgData.length > 2*byteNum*1024) {
...
+(int)getImageHeight:(UIImage *)image withImageWidth:(int)width{
    ...
    if (image) {
        ...
        float i = (float)width *((float)image.size.height/(float)image.size.width);
        height = (int)i;
    }
    return height;
}
